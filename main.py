...
last_update_id = 0
vehicle_detection_time = {}  # Track khi xe vào bán kính 1.5km lần đầu
    ...
    for vehicle in vehicles:
        try:
            ...
            if current_speed > 3:
                ...
                # Chỉ báo 1 lần duy nhất: dừng >= 5 phút và chưa báo lần nào
                should_notify = False
                if stopped_duration >= SIGNAL_LOSS_THRESHOLD and not status['notified']:
                    should_notify = True
                    ...
                    if dist <= DETECTION_RADIUS_FAR:
                        ...
                        if key not in notified or (current_time - notified[key]).total_seconds() > 600:
                            ...
                            if route == 'unknown' or route == expected_route:
                                detection_key = f"{plate}_{station_name}_{box_key}"
                                
                                # Tracking lần đầu xe vào bán kính 1.5km
                                if detection_key not in vehicle_detection_time:
                                    vehicle_detection_time[detection_key] = current_timestamp
                                    # Bỏ qua lần đầu, chưa báo - chờ 90s
                                    continue
                                
                                # Chỉ báo nếu đã chờ >= 90 giây
                                detection_elapsed = current_timestamp - vehicle_detection_time[detection_key]
                                if detection_elapsed < 90:
                                    continue
                                ...
                                if send_telegram(msg, chat_id):
                                    ...
                                    daily_stats[current_time.date()] += 1
                                    del vehicle_detection_time[detection_key]  # Xóa tracking
